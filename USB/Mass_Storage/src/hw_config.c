...
void USB_To_USART_Send_Data(uint8_t* data_buffer, uint8_t Nb_bytes)	//从USB接收到数据 -> 串口
{
  ...
  transmitter_handle_upstream(data_buffer, Nb_bytes);
...
void Handle_USBAsynchXfer (void)	//USB发送数据给上位机
{
  ...
  uint16_t USB_Tx_length;
	...
	struct xmit_list *pxmit_buf;
	pxmit_buf = xmit_list_pop();
	/* No packet needs to be transmitted, just transfer the SYNC packet */
	if(pxmit_buf){
		UserToPMABufferCopy(pxmit_buf->packet, ENDP1_TXADDR, pxmit_buf->packet_len);
		SetEPTxCount(ENDP1, pxmit_buf->packet_len);
		SetEPTxValid(ENDP1); 	
		free(pxmit_buf->packet);
		free(pxmit_buf);
	}
